...
function computed(getter) {
  // 1. 创建一个响应式变量 result
  // 2. 在 effer 中运行 getter, 设置 result.value
  // 3. 返回 result
  let result = ref()
...
console.log(`before update the total (should be 9 ) = ${total.value} salePrice (should be 4.5) = ${salePrice.value}`);

product.quantity = 3
console.log(`after update the total (should be 13.5 ) = ${total.value} salePrice (should be 4.5) = ${salePrice.value}`);

product.price = 10
// 会触发两个effect
console.log(`after update the total (should be 27 ) = ${total.value} salePrice (should be 9) = ${salePrice.value}`);
